...
import { useIntAnimation } from "./util/useAnimate.js";
import { useTimeout } from "./util/useTimeout.js";
import { EDGE_LEFT, EDGE_TOP, useYogaNode } from "./util/useYogaNode.js";

type ScrollbarCommonProps = {
	style?: {
		track: string;
...
const DefaultHScrollbarStyle: HScrollbarProps["style"] = {
	track: "▁",
	thumb: "▂▂",
};

export const HScrollbar_ = forwardRef<DOMElement, HScrollbarProps>(
	(
		{
			scrollLeft,
			scrollWidth,
			width,
			style = DefaultHScrollbarStyle,
			visible = "onscroll",
			animate,
			...props
		},
		...
		return (
			<Box
				...
				{...props}
			>
				<Text color={style.trackColor}>{trackBar}</Text>
...
export const useScrollbar = ({
	...
	const trackBar = useMemo(() => style.track.repeat(size), [size]);
